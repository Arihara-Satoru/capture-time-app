...
重复清理会话目录包含 `deleted.tsv`，列为 `original_path`、`retained_path`、`backup_path`。该清单在文件删除后、后续核验前写入，记录已执行的删除，不代表媒体库核验通过。删除后的扫描、核验或日志错误单独显示为待确认，不再算作未删除；删除前失败不会删除原件。文件删除和日志写入之间仍有断电窗口。既有 `capture-time-app-*` 备份不会被重复清理功能删除。核验只保证检查时的状态，不能阻止外部云相册后续重新下载。
