...
7. 切换到“照片重复纠正”后点击“扫描严格重复项”。候选默认全选，可逐项取消；确认后每项都会重新核验真实路径、MediaStore 有效尺寸、实际大小和 SHA-256。
8. 每批使用新的 `/sdcard/.temp/duplicate-cleanup-日期时间/`，按原始相对路径备份。备份通过逐字节 `cmp` 等价校验和 SHA-256 校验后才删除原件，并写入 `deleted.tsv`；删除后会核验原路径、保留文件、备份文件并重新扫描。
...
设置页的“清除备份”只处理名称以 `capture-time-app-` 或 `duplicate-cleanup-` 开头的会话目录，并会在永久删除前显示目录数量和大致大小并要求二次确认。它会同时删除时间纠正与重复清理会话中的照片备份和日志，但不会删除 `/sdcard/.temp` 下的其他文件或目录。清除后无法依靠这些备份恢复照片。
...
YYYYMMDD_HHMMSS
YYYYMMDD-HHMMSS
13 位 Unix 毫秒时间戳，例如 1534600669491
```

无时区的格式化文件名和 EXIF 均按 `Asia/Shanghai` 严格解析；Unix 毫秒时间戳按绝对时间解析，并仅接受 2000 年至 2100 年范围内的值，避免把普通长编号误判为时间。非法日期会忽略；选择文件名作为依据时，一个文件名中出现多个不同有效时间会跳过。
...
重复清理会话目录只包含 `deleted.tsv`，列为 `original_path`、`retained_path`、`backup_path`。清理失败的项目不会写入成功清单，也不会删除原件；既有 `capture-time-app-*` 备份不会被重复清理功能删除。
