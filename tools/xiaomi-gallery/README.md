...
v1.5.0 也可直接从应用首页进入“小米相册时间修复 · Root”。界面与这个脚本共用 `app/src/main/java/local/capturetime/gallery/GalleryTimeRepair.java`，应用内备份可导出 ZIP。
...
- 沿用应用设置中“忽略误差”的阈值，仅修复相册拍摄或排序时间超过该误差的条目。无可靠文件名、来源冲突、视频和仅在云端的照片不自动处理。
